...
def testBuffer(values, buffer, expectVal):
    for v in values:
        isUpd, value = buffer.add(float(v), 0)
        if isUpd:
            if value != expectVal:
...
def main():
    ...
    print(b)
    if not isUpdate:
        print("Error expexted update and new value: {}".format(value))
    else:
        if value != 2:
            print("Error expect 2 got {}".format(value))
    if b.fregIx != 0 or b.ix != 0 or b.sum != 4 or b.no != 2:
        print("Error Failed add")

    isUpdate, value = b.add(3.0, 1)
    print(3)
    print(b)
    isUpdate, value = b.add(4.0, 1)
    print(4)
    print(b)
    isUpdate, value = b.add(5.0, 1)
    print(5)
    print(b)
    if not isUpdate:
        print("Error expexted true")
    else:
        if value != 4:
            print("Error expect 4 got {}".format(value))
